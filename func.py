...
class devops:
    ...
    def invoke_deployment(self, image_version, oci_deployment_pipeline_id):
        try:
            ...
            create_deployment_response = devops_client.create_build_run(
                create_build_run_details=oci.devops.models.CreateBuildRunDetails(
                build_pipeline_id=oci_deployment_pipeline_id,
...
def handler(ctx, data: io.BytesIO=None):
    try:
        ...
        logging.getLogger().info("New image version is {image_version} and image digest is {image_digest}")
